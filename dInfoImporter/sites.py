...
class Melonbooks(Site):
    ...
    def import_url(self, url: str) -> Doujinshi:
        ...
        d.is_adult = True if td_extractor('作品種別').text.strip() == '18禁' else False
        ...
        d.coupling_names = []
        d.character_names = []
        d.is_novel = True if '小説' in d.general_tags else False
        d.is_anthology = True if 'アンソロジー' in d.general_tags else False
...
class Toranoana(Site):
    ...
    def import_url(self, url: str) -> Doujinshi:
        ...
        d.is_adult = True if soup.find('span', class_='mk-rating') else False
        ...
        if td_type is not None:
            ...
            d.is_copybook = True if type_list[0] == 'コピー誌' else False
            d.is_novel = True if type_list[1] == '小説' else False
